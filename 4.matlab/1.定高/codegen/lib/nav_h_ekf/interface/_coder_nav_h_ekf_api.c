...
static float (*i_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src,
  const emlrtMsgIdentifier *msgId))[16];
static float (*j_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src,
 ...
 * Return Type  : void
 */
void nav_h_ekf_api(const mxArray *prhs[6], const mxArray *plhs[3])
{
  float (*Q)[16];
  float (*H)[8];
  float (*R)[4];
